...
async def get_phone_verifycation_by_otp_hash(
    otp_hash: str, db: AsyncSession
) -> PhoneVerificationOtp:
    result = await db.execute(
        select(PhoneVerificationOtp).where(PhoneVerificationOtp.otp_hash == otp_hash)
    )
    return result.scalar_one_or_none()


async def get_phone_verifycation_by_phone(
    phone: str, db: AsyncSession
) -> PhoneVerificationOtp:
    result = await db.execute(
        select(PhoneVerificationOtp).where(PhoneVerificationOtp.phone == phone)
    )
    return result.scalar_one_or_none()
...
async def get_attempts_phone_verifycation_by_phone(phone: str, db: AsyncSession):
    result = await db.execute(
        select(PhoneVerificationOtp.attempts).where(PhoneVerificationOtp.phone == phone)
    )
    return result.scalar_one_or_none()
