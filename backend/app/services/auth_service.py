...
async def login(
    ...
    if not user: user = await get_user_by_phone(phone=identifier, db=db)
    if not user: user = await get_user_by_username(username=identifier, db=db)
...
async def logout(refresh_token: str, db: AsyncSession):
    ...
    await revoke_session(session.id, datetime.now(UTC).replace(tzinfo=None), "logout", db)
