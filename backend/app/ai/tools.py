...
def get_agent_tools(db: AsyncSession) -> List[Any]:
    ...
    async def search_catalog(
        ...
        try:
            ...
            products, _ = await product_repository.get_public_products(
                db,
                keyword=keyword,
            ...
            for p in products:
                ...
                prices = [
                    float(v.price) for v in active_variants if getattr(v, "price", None) is not None
                ]
                sale_prices = [
                    float(v.sale_price)
                ...
                pub_id = getattr(first_variant, "public_id", None) if first_variant else None
                var_id = getattr(first_variant, "id", None) if first_variant else None
