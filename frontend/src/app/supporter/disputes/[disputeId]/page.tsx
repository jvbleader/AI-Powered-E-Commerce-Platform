...
export default function AdminDisputeDetailPage() {
    ...
    return (
      <div className="py-8 max-w-2xl mx-auto">
        <ErrorState
          ...
          action={
            <Button variant="secondary" onClick={() => router.push("/supporter/disputes")}>
  ...
  const hasDisputed = isDisputed || isApproved || isRejected || !!dispute.disputedAt;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-500">
              {hasDisputed && dispute.disputedAt ? (
                <>
                ...
                <>
                  Thời gian tạo yêu cầu trả hàng:{" "}
                  <span className="font-semibold text-slate-700">
      ...
      <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="rounded-xl border border-slate-100 bg-slate-50/50 p-3.5 text-xs space-y-1.5">
                  ...
                  <p className="text-slate-700">
                    Họ tên: <strong>{(() => {
                      const rName = order.receiverName || order.shipment?.receiverName;
                      if (rName && rName.trim() !== "-" && rName.trim() !== "") return rName.trim();
                      if (dispute.user?.fullName && dispute.user.fullName.trim() !== "") return dispute.user.fullName.trim();
                      if ((dispute as any).order?.user?.fullName) return (dispute as any).order.user.fullName;
                      return `Khách #${dispute.userId}`;
                    })()}</strong>
                  </p>
                  {(order.phone || order.shipment?.receiverPhone) && (
                    <p className="text-slate-700">
                      Số điện thoại: <strong>{order.phone || order.shipment?.receiverPhone}</strong>
                    </p>
                  )}
                  {(order.shippingAddress || (order.shipment ? `${order.shipment.detailAddress}, ${order.shipment.ward}, ${order.shipment.district}, ${order.shipment.province}` : null)) && (
                    <p className="text-slate-600 text-[11px] leading-relaxed">
                      Địa chỉ nhận: {order.shippingAddress || `${order.shipment.detailAddress}, ${order.shipment.ward}, ${order.shipment.district}, ${order.shipment.province}`}
                    </p>
                  )}
                </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
          ...
          <div className="relative pl-6 space-y-6 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
            ...
            <div className="relative">
              ...
              <div className="space-y-0.5">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="text-xs font-bold text-slate-900">4. Đã giao hàng</span>
                  <span className="text-[11px] font-medium text-slate-500">
                    {(() => {
                ...
                <div className="space-y-0.5">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-xs font-bold text-indigo-950">
                      7. Người mua khiếu nại lên Sàn
                    </span>
                    <span className="text-[11px] font-medium text-slate-500">
                      {dispute.disputedAt ? formatDate(dispute.disputedAt) : "Đã gửi khiếu nại"}
                    </span>
                  </div>
                  <p className="text-xs text-indigo-900">
                    Nội dung khiếu nại: "{dispute.disputeReason || "Yêu cầu can thiệp đối soát từ Supporter Sàn."}"
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="rounded-xl border border-sky-200 bg-sky-50/40 p-4 space-y-3.5">
              <div className="flex items-center justify-between border-b border-sky-200/80 pb-2">
                ...
                <span className="text-[11px] text-sky-800 bg-sky-100 font-semibold px-2 py-0.5 rounded">
                  {hasDisputed ? "Khiếu nại sàn" : "Yêu cầu trả hàng"}
                </span>
              </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm space-y-4">
          ...
          {isDisputed ? (
                ...
                <div className="rounded-xl border border-emerald-200 bg-emerald-50/30 p-4 flex flex-col justify-between gap-3">
                  <div>
                    <h4 className="text-sm font-bold text-emerald-950 flex items-center gap-1.5">
                      ...
                      <span>Chấp thuận khiếu nại, hoàn 100% giá trị đơn hàng và không cần hoàn hàng về shop</span>
                    </h4>
                ...
                <div className="rounded-xl border border-rose-200 bg-rose-50/30 p-4 flex flex-col justify-between gap-3">
                  <div>
                    <h4 className="text-sm font-bold text-rose-950 flex items-center gap-1.5">
                      ...
                      <span>Bác bỏ khiếu nại, giữ nguyên quyết định của shop, tự động hoàn tất đơn hàng</span>
                    </h4>
                    <p className="text-xs text-slate-600 mt-1 leading-relaxed">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-slate-900">
                      {isApproved
                        ? "Phán quyết: Chấp thuận khiếu nại, hoàn 100% giá trị đơn hàng và không cần hoàn hàng về shop"
                        : "Phán quyết: Bác bỏ khiếu nại, giữ nguyên quyết định của shop, tự động hoàn tất đơn hàng"}
                    </h3>
                    <p className="text-[11px] text-slate-500">
            ...
            /* NOT ESCALATED YET (e.g. SELLER_REJECTED / REQUESTED / SELLER_APPROVED) */
            <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-5 text-center space-y-2">
              <div className="flex justify-center text-slate-400">
                <Clock className="h-6 w-6" />
              </div>
              <h4 className="text-sm font-bold text-slate-800">
                Chưa có yêu cầu khiếu nại lên Sàn
              </h4>
              <p className="text-xs text-slate-500 max-w-md mx-auto leading-relaxed">
                Yêu cầu đổi trả này hiện đang ở trạng thái <strong>{returnStatusLabel[dispute.returnStatus] || dispute.returnStatus}</strong>.
                Người mua chưa gửi khiếu nại lên Sàn, do đó Supporter chưa cần thực hiện phán quyết đối soát.
              </p>
            </div>
          )}
        </div>
