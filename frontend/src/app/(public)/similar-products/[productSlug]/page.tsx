...
export default function SemanticSimilarProductsPage() {
  ...
  const limit = 48; // Updated to 48 as requested
  ...
  useEffect(() => {
    ...
    const fetchSimilar = async () => {
      try {
        ...
        // Call the new paginated API
        const res = await recommendationService.getSimilarProducts(productSlug, limit, page);
        if (isMounted) {
  ...
  if (!productSlug) return <NotFoundPage />;
