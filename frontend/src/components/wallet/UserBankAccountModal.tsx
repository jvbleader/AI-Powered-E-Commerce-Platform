...
import type { BankAccountInfo, UpdateWalletBankAccountRequest } from "@/services/wallet-api";

interface UserBankAccountModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialBankInfo?: BankAccountInfo | null;
  onSubmit: (payload: UpdateWalletBankAccountRequest) => Promise<boolean>;
}
...
export function UserBankAccountModal({
  open,
  onOpenChange,
  initialBankInfo,
  onSubmit,
}: UserBankAccountModalProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="mt-5 flex items-center justify-end gap-2 pt-2 border-t border-line/60">
            ...
            <Button
              type="submit"
              disabled={submitting}
              className="bg-primary hover:bg-primary/90 text-white h-9 text-xs font-bold"
            >
