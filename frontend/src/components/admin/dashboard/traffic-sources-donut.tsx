...
import { AdminTrafficSourceItem } from "@/services/admin-api";

interface TrafficSourcesDonutProps {
  data: AdminTrafficSourceItem[];
  totalVisits: number;
  loading?: boolean;
}
...
export function TrafficSourcesDonut({
  ...
  const CustomTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      const item: AdminTrafficSourceItem = payload[0].payload;
      return (
        <div className="rounded-lg border border-slate-200 bg-white/95 p-2 text-xs shadow-md backdrop-blur-sm">
          ...
          <div className="text-slate-600 mt-1">
            <strong>{item.count.toLocaleString("vi-VN")}</strong> lượt ({item.percentage}%)
          </div>
        </div>
      );
    }
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
      <div className="mb-2">
        <h3 className="text-sm font-bold text-slate-900">Nguồn truy cập</h3>
        <p className="text-xs text-slate-600">Phân loại các kênh mang lại lượt truy cập</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-2.5 items-center my-auto">
        ...
        <div className="sm:col-span-7 space-y-1 text-xs">
          {data.map((item) => (
            <div key={item.channel} className="flex items-center justify-between gap-1.5 py-0.5 border-b border-slate-50 last:border-0">
