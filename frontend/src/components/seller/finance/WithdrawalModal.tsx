...
import { Wallet, AlertCircle, ArrowRight, Building2, X } from "lucide-react";
import type { SellerWalletData } from "@/store/slices/types";

interface WithdrawalModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  wallet: SellerWalletData | null;
  onSubmitWithdraw: (amount: number) => Promise<boolean>;
...
export function WithdrawalModal({
  ...
  const handleSelectMax = () => {
    setAmountStr(Math.floor(availableBalance).toLocaleString("vi-VN"));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const ok = await onSubmitWithdraw(amountNum);
      if (ok) {
        setAmountStr("");
        setErrorMsg("");
        onOpenChange(false);
    ...
    >
      <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl border border-line p-6 relative">
        ...
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-base font-bold text-ink">Rút tiền từ Ví người bán</h3>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="text-[11px] text-slate-500 space-y-1 bg-slate-50 p-2.5 rounded-lg border border-line/60">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-slate-700">Tức thì qua Napas 24/7</span>
            </div>
          </div>
          ...
          <div className="mt-5 flex items-center justify-end gap-2 pt-2 border-t border-line/60">
            ...
            <Button
              type="submit"
              disabled={submitting || !hasBankInfo || amountNum < 50000 || amountNum > availableBalance}
              className="bg-emerald-600 hover:bg-emerald-700 text-white flex items-center gap-1.5 h-9 text-xs"
            >
